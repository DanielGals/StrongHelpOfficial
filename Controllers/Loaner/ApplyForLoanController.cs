...
namespace StrongHelpOfficial.Controllers.Loaner
{
    public class ApplyForLoanController : Controller
    {
        ...
        public IActionResult testLang(ApplyForLoanViewModel model)
        {
            for (int i = 0; i < model.LoanDocumentName.Length; i++)
            {
                TempData["test"] = $"loan amount: {model.LoanAmount} loan file: {model.Filecontent[i]} loanfile name: {model.LoanDocumentName[i]}\ngood job";
            }
        ...
        {
            // This action can be used to display the result of the submission
            // For now, it just returns a view with the model data
        ...
        public async Task<IActionResult> UploadDocuments(ApplyForLoanViewModel model)
        {
            ...
            */
            ...
            using (var conn = new SqlConnection(_config.GetConnectionString("DefaultConnection")))
            {
                ...
                //temporary code placement for these
